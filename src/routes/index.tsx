...
function HomePage() {
  ...
  const weekdayLabel = today.charAt(0).toUpperCase() + today.slice(1);

  const menu = menuData as MenuDish[];

  // ✅ TODAY'S MENU (clickable cards)
  const todaysMenu = menu.filter((item) => isAvailableToday(item, today));

  // ✅ FULL WEEK MENU (sorted: today's dishes first)
  const fullWeekMenu = [...menu].sort((a, b) => {
    const aToday = isAvailableToday(a, today) ? 1 : 0;
    const bToday = isAvailableToday(b, today) ? 1 : 0;
        ...
        <div className="flex items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-1 font-display text-3xl text-foreground sm:text-4xl">
              Available today — {weekdayLabel}
            </h2>
            <p className="mt-3 max-w-2xl text-muted-foreground">
              These dishes are available for ordering right now.
            </p>
          </div>
        ...
        {todaysMenu.length === 0 ? (
          <div className="mt-8 rounded-2xl border border-border bg-card p-8 text-center">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Please check the full week menu below to see what is available on
              other days.
            </p>
          </div>
        ) : (
      ...
      <section className="border-y border-border bg-card/50">
        <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-muted-foreground">
              Dishes marked <strong>Available today</strong> are ready to order
              now. The rest show the days they appear during the week.
            </p>
          </div>

          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {fullWeekMenu.map((dish) => {
              const availableToday = isAvailableToday(dish, today);
