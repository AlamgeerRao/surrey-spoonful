...
type Category = "curries" | "rice" | "vegetarian" | "sides";
...
function findDishBySlug(slug: string): Dish | undefined {
...
export const Route = createFileRoute("/menu/$slug")({
  loader: ({ params }): { dish: Dish } => {
    const dish = findDishBySlug(params.slug);
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          {
            title: `${loaderData.dish.name} — ZAIQA | Homemade Pakistani Kitchen`,
          },
      ...
      : [],
    links: loaderData
      ? [{ rel: "canonical", href: `/menu/${loaderData.dish.slug}` }]
      : [],
  }),
...
function MealPage() {
  ...
  const size = useMemo(
    () => dish.sizes.find((s) => s.id === sizeId) ?? dish.sizes[0],
    [dish, sizeId]
  );
        ...
        <div>
          ...
          <dl className="mt-6 grid grid-cols-2 gap-4 text-sm">
            ...
            <Meta label="Allergens">
              {dish.allergens.length ? dish.allergens.join(", ") : "None declared"}
            </Meta>
            ...
            <Button
              ...
              onClick={() => {
                addToCart({
                  ...
                  name: dish.name,
                  size: size.label,
                  pricePence: size.pricePence,
                  quantity: qty,
                });

                toast.success(`Added ${qty} × ${dish.name} (${size.label})`);
              }}
            >
