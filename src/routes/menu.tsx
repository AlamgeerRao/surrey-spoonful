...
import { DISHES, CATEGORIES, type Category } from "@/lib/menu-data";
...
function MenuPage() {
  ...
  const items = active === "all" ? DISHES : DISHES.filter((m) => m.category === active);
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
      ...
      <div className="sticky top-16 z-30 -mx-4 mb-8 overflow-x-auto border-b border-border bg-background/85 px-4 py-3 backdrop-blur sm:-mx-6 sm:px-6">
        <div className="flex gap-2">
          ...
          {CATEGORIES.map((c) => (
            <CatPill key={c.id} label={c.label} active={active === c.id} onClick={() => setActive(c.id)} />
          ))}
        </div>
      </div>

      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((m) => (<MealCard key={m.id} item={m} />))}
      </div>
